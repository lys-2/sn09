...
/* Set socket timeouts to avoid blocking */
void set_socket_timeouts(int sockfd) {
    struct timeval tv;
...
/* Handle one HTTP request (called from child process) */
void handle_client(int client) {
    set_socket_timeouts(client);   /* prevent infinite recv/send */
...
int main() {
    /* Ignore SIGPIPE (broken pipe) to avoid crashing on write to closed socket */
    ...
    while (1) {
        ...
        if (pid == 0) {
            ...
            /* handle_client never returns – it calls exit() */
        }
        /* Parent closes the client socket and continues accepting */
        close(client);
    }
